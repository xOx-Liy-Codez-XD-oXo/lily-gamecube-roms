...
void setupCircle(float * verts, int resolution) {
	int tranglefanverts = 1 + resolution; //for loopround
	verts[0] = 0.0f;
	verts[1] = 1.0f;
	for(int i = 0; i < tranglefanverts; i++) {
		int i2 = i*2; 
		i2+=2;
		float faralong = (float)(i + 1) / (float)(tranglefanverts - 1) * 6.2831853072f;
		verts[i2] = sin(faralong);
		verts[i2+1] = cos(faralong);
	}
}
...
void drawCircle(float *verts, int resolution) {
	int tranglefanverts = resolution; //for loopround
	GX_Begin(GX_TRIANGLEFAN, GX_VTXFMT0, tranglefanverts);
	for(int i = 0; i < tranglefanverts; i++) {
		int i2 = i * 2;
		GX_Position2f32(verts[i2], verts[i2+1]);
	}
}
